...
#endif
{
 ...
 if (argc==7 || argc==10)
 {
  ...
  for (int j=0; j<Nz; j++)
  {
   ...
   double *p1=Parms_ext+j*InSize_ext;
  ...
  if (argc==7)
  {
   ...
   res=MW_Transfer(Lparms, Rparms, Parms_int, T_arr, DEM_arr, DDM_arr, RL, 0, 0, 0, 0, 0, 0);
  }
  else
  {
   double *fZ_arr=(double*)argv[6];
   ...
   res=MW_Transfer(Lparms, Rparms, Parms_int, T_arr, DEM_arr, DDM_arr, RL, 1, fZ_arr, TZ_arr, Z_arr, 0, 0);
  }
...
#endif
{
 ...
 concurrency::parallel_for(0, Npix, [&](int pix)
 {
  ...
  ARGV[2]=(void*)(Parms_M+pix*Nz*InSize_ext);
  ARGV[3]=(void*)T_arr;
...
#endif
{
 ...
 { 
  ...
  int res=MW_Transfer(Lparms, Rparms, Parms_int, T_arr, DEM_arr, DDM_arr, RL, 0, 0, 0, 0, srange, GRparms);
...
#endif
{
 ...
 if (argc>=9)
 {
  ...
  double *Parms_o=(double*)malloc(sizeof(double)*Nz*InSize_ext);
  memcpy(Parms_o, Parms, sizeof(double)*Nz*InSize_ext);
  ...
  for (int i=0; i<Nz; i++)
  {
   double Q=Parms[D2(InSize_ext, 11, i)];
   double L=Parms[D2(InSize_ext, 12, i)];
   ...
   if (Q>0 && L>0 && (DEM_arr!=0 || DDM_arr!=0))
   {
    ...
    Parms_o[D2(InSize_ext, 11, i)]=!(DEM_arr!=0 && ok!=0);
    Parms_o[D2(InSize_ext, 12, i)]=!(DDM_arr!=0 && ok!=0);
   }
   else Parms_o[D2(InSize_ext, 11, i)]=Parms_o[D2(InSize_ext, 12, i)]=1;
  }
